 ...
 * <p>
 * In general, methods of this class have three ways of failing. They will throw
 * IllegalArgumentException on usage errors, IOException on network errors or
 * other errors that may be reasonably expected to succeed on retry, and return
 * false for conditions where retry will not succeed (e.g. permission errors,
 * buckets and items not existing, etc).
 * <p>
...
public class S3Store {
	 ...
	 * 
	 * @param copy
	 *            The S3Store instance to copy [may not be null]
	 **/
	 ...
	 * constructed instance, so if any connection parameters are bad, it won't
	 * be known until then.
	 * 
	 * @param host
	 *            The S3 storage host to connect to [may not be null]
	 * @param username
	 *            The Amazon Web Services Access Key ID to use to connect [may
	 *            not be null]
	 * @param password
	 *            The Amazon Web Services Secret Access Key to use to connect
	 *            [may not be null]
	 **/
	public S3Store(final String host, final String username,
	...
	/**
	 * Standard constructor taking connection parameters and a default bucket to
	 * use. No connection is made until an instance method of this class is
	 * called, so if these parameters are incorrect it won't be known until
	 * then.
	 * 
	 * @param host
	 *            The S3 storage host to connect to [may not be null]
	 ...
	 *            [may not be null]
	 * @param bucket
	 *            The S3 bucket to use [may be null, although some operations
	 *            will fail]
	 **/
	public S3Store(final String host, final String username,
			final String password, final String bucket) {
	 ...
	 * 
	 * @param bucket
	 *            The bucket to use [may be null, although some operations will
	 *            fail]
	 **/
	 ...
	 *             If there is no bucket set
	 * @throws IOException
	 *             From underlying network problems or if S3 returned an
	 ...
	 * 
	 * @return A List of Strings of item ids in this bucket or null if there was
	 *         an error.
	 * @throws IOException
	 *             From underlying network problems or if S3 returned an
	 *             internal server error.
	 ...
	 * 
	 * @param id
	 *            The ID to store the item to [may not be null]
	 * @param data
	 *            The binary data to store [may not be null]
	 * @return True if the operation succeeded, false if it failed.
	 * @throws IllegalArgumentException
	 *             If there is no bucket set
	 * @throws IOException
	 ...
	 *             internal server error.
	 **/
	public boolean storeItem(final String id, final byte[] data)
			throws IOException {
		return storeItem(id, data, (Map<String, List<String>>) null);
	}

	private void addAclHeader(final Map<String, List<String>> headers,
			final String acl) {
	 ...
	 *            The binary data to store [may not be null]
	 * @param acl
	 *            convenience param to specify an acl. equivalent to including a
	 *            header of "x-amz-acl" with this value. Must be one of
	 *            public-read, public-write, authenticated-read, or private (the
	 *            default). See:
	 *            http://docs.amazonwebservices.com/AmazonS3/latest
	 *            /index.html?S3_ACLs.html for more info.
	 * @param headers
	 *            other headers to send. may be null or empty. useful for
	 *            setting content-type, acls, or other user meta-data. see
	 *            http://docs.amazonwebservices.com/AmazonS3/latest/index.html?
	 *            UsingMetadata.html for more info.
	 * @return True if the operation succeeded, false if it failed.
	 * @throws IllegalArgumentException
	 *             If there is no bucket set
	 * @throws IOException
	 ...
	 **/
	public boolean storeItem(final String id, final byte[] data,
			final String acl, final Map<String, List<String>> _headers)
			throws IOException {
		final Map<String, List<String>> headers = new HashMap<String, List<String>>();
	...
	public boolean storeItem(final String id, final byte[] data,
		...
		final HttpURLConnection itemConn = getItemURLConnection("PUT", id,
				data, headers);
	...
	public boolean copyItem(final String fromKey, final String toKey,
			final String acl) throws IOException {
		if (fromKey == null || toKey == null) {
			throw new IllegalArgumentException(
					"neither fromKey or toKey can be null");
		}
		...
		final HttpURLConnection itemConn = getItemURLConnection("PUT", toKey,
				null, headers);
		itemConn.connect();
	...
	public Map<String, List<String>> getMeta(final String key)
			throws IOException {
		if (key == null) {
			throw new IllegalArgumentException("key must not be null");
		}

		final HttpURLConnection itemConn = getItemURLConnection("HEAD", key,
				null, null);

		if (!checkResponse("getMeta", itemConn))
			return null;
	 ...
	 * 
	 * @param id
	 *            The item to get [may not be null]
	...
	public S3Object getItemWithHeaders(final String id) throws IOException {
		...
		final HttpURLConnection itemConn = getItemURLConnection("GET", id,
				null, null);

		itemConn.connect();

		if (!checkResponse("getItem", itemConn))
			return null;
	 ...
	 * 
	 * @param id
	 *            The item to delete [may not be null]
	...
	public boolean deleteItem(final String id) throws IOException {
		...
		final HttpURLConnection itemConn = getItemURLConnection("DELETE", id,
				null, null);
	...
	/**
	 * Lists those contents of the current bucket with IDs starting with the
	 * given prefix.
	 * 
	 * @param prefix
	 *            The prefix to limit searches to. If null, no restriction is
	 *            applied.
	 * @return A list of ids or null if there was an error.
	 * @throws IllegalArgumentException
	 *             If there is no bucket set
	...
	/**
	 * Lists those contents of the current bucket with IDs starting with the
	 * given prefix that occur strictly lexicographically after the the given
	 * marker.
	 * 
	 * @param prefix
	 *            The prefix to limit searches to. If null, no restriction is
	 *            applied.
	 * @param marker
	 *            The marker indicating where to start returning results. If
	 *            null, no restriction is applied.
	 * @throws IllegalArgumentException
	 *             If there is no bucket set
	 * @throws IOException
	 *             From underlying network problems or if S3 returned an
	 *             internal server error.
	 **/
	public List<String> listItems(final String prefix, final String marker)
			throws IOException {
	...
	/**
	 * Lists those contents of the current bucket with IDs starting with the
	 * given prefix that occur strictly lexicographically after the given
	 * marker, limiting the results to the given maximum number.
	 * 
	 * @param prefix
	 *            The prefix to limit searches to. If null, no restriction is
	 *            applied.
	 ...
	 *            null, no restriction is applied.
	 * @param max
	 *            The maximum number of results to return. If 0, no additional
	...
	public List<String> listItems(final String prefix, final String marker,
		...
		final HttpURLConnection bucketConn = getBucketURLConnection("GET",
				prefix, marker, max);

		bucketConn.connect();

		if (!checkResponse("listItems(" + prefix + "," + marker + "," + max
				+ ")", bucketConn)) {
			return null;
		}
	 ...
	 * succeeded or not. A request is a success if it returns a success response
	 * code (generally "200 OK" or "204 No Content", depending on the specific
	 * operation). In most cases, this method returns true on success, false on
	 * non-recoverable failure, or throws an IOException in cases where a retry
	 * might be reasonably expected to succeed. When this method returns false,
	 * it will also print an error message to the logger.
	 **/
	private boolean checkResponse(final String operation,
	...
	/**
	 * If a connection to S3 returned an error response code, this method will
	 * parse the error response XML and send the user-visible message to the
	 * logger as SEVERE.
	 **/
	...
	/**
	 * Creates a new HttpURLConnection that refers to the S3 root level for the
	 * "list buckets" operation.
	 **/
	private HttpURLConnection getRootURLConnection(final String method)
			throws IOException {
		final String url = "http://" + m_host + "/";
		final URL rootURL = new URL(url);

		final HttpURLConnection rootConn = (HttpURLConnection) rootURL
				.openConnection();
	 ...
	 * operations such as bucket creation, bucket deletion, and listing bucket
	 * contents.
	 **/
	private HttpURLConnection getBucketURLConnection(final String method)
			throws IOException {
	 ...
	 **/
	private HttpURLConnection getBucketURLConnection(final String method,
			final String prefix, final String marker, final int max)
			throws IOException {
		if (m_bucket == null) {
		...
		final HttpURLConnection bucketConn = (HttpURLConnection) bucketURL
				.openConnection();
	...
	/**
	 * Gets an HttpURLConnection referring to a specific item for storing and
	 * retrieving of data.
	 **/
	private HttpURLConnection getItemURLConnection(final String method,
			final String id, final byte[] data,
			final Map<String, List<String>> headers) throws IOException {
		if (m_bucket == null) {
			throw new IllegalArgumentException("bucket is not set");
		}

		final URL itemURL = new URL("http://" + m_host + "/" + m_bucket + "/"
				+ id);

		final HttpURLConnection urlConn = (HttpURLConnection) itemURL
				.openConnection();
	...
	public String getContentType(final HttpURLConnection conn) {
		for (final Map.Entry<String, List<String>> me : conn
	 ...
	 * authentication data to it to connect to S3. If connection data is
	 * provided, an MD5 digest is included for additional security, but the data
	 * itself is not written to the connection.
	 **/
	private void addAuthorization(final HttpURLConnection conn,
			final String method, final byte[] data) throws IOException {
		...
		if (data != null) {
			try {
				...
				throw new IllegalArgumentException(
						"unable to compute content-md5", e);
			}
		...
		try {
			final SecretKeySpec signingKey = new SecretKeySpec(
					m_password.getBytes(), SIGNATURE_ALGORITHM);
		...
		conn.setRequestProperty("Authorization", "AWS " + m_username + ":"
				+ auth);
	}
	...
	/**
	 * This is our simple SAX parser for handling response XML from S3. In every
	 * case where we parse an XML response, we're looking for the contents of a
	 * single tag which may occur multiple times, such as "key" tags when
	 * listing bucket contents or "message" tags when parsing error responses.
	 * This class assumes that the target tags don't contain any subtags.
	 **/
	private static class ObjectListParser extends DefaultHandler {
		...
		/**
		 * Constructs an object list parser that gather the contents of tags
		 * with the given name.
		 * 
		 * @param lookfor
		 *            The element name to gather character data from
		 **/
		...
		/**
		 * Once parsing is complete, this retrieves the list of contents of all
		 * the matching tags encountered.
		 * 
